defmodule Rfx.Source do
  ...
  def diff({src1, src2}) do
    path1 = @base_diff <> "/src1_" <> Util.rand_str()
    path2 = @base_diff <> "/src2_" <> Util.rand_str()
  ...
  def patch(source, diff) do
    spath = @base_diff <> "/patch_src_"  <> Util.rand_str()
    dpath = @base_diff <> "/patch_diff_" <> Util.rand_str()
